...
import { getSystemPrompt, EXTRACTION_VISION_SYSTEM, EXTRACTION_VISION_USER_PROMPT, EXTRACTION_REVIEW_USER_PROMPT, PLAN_TEXT_EXTRACTION_PROMPT, PLAN_TEXT_AND_COORDINATES_PROMPT } from './base-prompts';
...
export async function runPipeline(params: OrchestratorParams): Promise<PipelineResult> {
  ...
  if (hasKey) {
    try {
      ...
      // Optional: second-pass review (multilook) — same image + first-pass JSON, model corrects and adds missing rooms
      if (
        process.env.ENABLE_EXTRACTION_REVIEW_PASS === 'true' &&
        useVisionPrompt &&
        imageUrlForVision
      ) {
        let firstPassRooms: RoomForValidation[] | undefined;
        let firstPassCanvas: { width?: number; height?: number } | undefined;
        try {
          const firstParsed = JSON.parse(extractJson(extractionContent)) as Record<string, unknown>;
          firstPassRooms = firstParsed.rooms as RoomForValidation[] | undefined;
          firstPassCanvas = firstParsed.canvas_size as { width?: number; height?: number } | undefined;
        } catch {
          firstPassRooms = undefined;
        }
        const hasRoomsSchema =
          Array.isArray(firstPassRooms) &&
          firstPassRooms.length > 0 &&
          (firstPassRooms[0] as { box_2d?: unknown })?.box_2d &&
          firstPassCanvas;
        if (hasRoomsSchema) {
          const reviewPrompt = `${EXTRACTION_REVIEW_USER_PROMPT}\n\nFirst-pass extraction (review and correct this):\n${extractionContent.slice(0, 4000)}`;
          const reviewMessages = [
            { role: 'system' as const, content: EXTRACTION_VISION_SYSTEM },
            {
              role: 'user' as const,
              content: [
            ...
            const reviewContent =
              typeof review.content === 'string'
                ...
                : Array.isArray(review.content)
                  ? (review.content as Array<{ type?: string; text?: string }>)
                      .map((p) => (p?.type === 'text' && typeof p?.text === 'string' ? p.text : ''))
                      .join('')
                  : String(review.content ?? '');
            const reviewParsed = (() => {
              try {
                return JSON.parse(extractJson(reviewContent)) as Record<string, unknown>;
              } catch {
                return null;
              }
            })();
            const reviewRooms = reviewParsed?.rooms as RoomForValidation[] | undefined;
            const reviewCanvas = reviewParsed?.canvas_size as { width?: number; height?: number } | undefined;
            const reviewW = typeof reviewCanvas?.width === 'number' && reviewCanvas.width > 0 ? reviewCanvas.width : 1000;
            const reviewH = typeof reviewCanvas?.height === 'number' && reviewCanvas.height > 0 ? reviewCanvas.height : 1000;
            const reviewErrors =
              Array.isArray(reviewRooms) && reviewRooms.length > 0 && reviewCanvas
                ? validateFloorplan(reviewRooms, reviewW, reviewH)
                : ['Invalid schema'];
            const reviewValid = reviewErrors.length === 0;
            const firstCount = firstPassRooms?.length ?? 0;
            const reviewCount = Array.isArray(reviewRooms) ? reviewRooms.length : 0;
            if (reviewValid && reviewCount >= firstCount) {
              extractionContent = reviewContent;
              if (review.usage) {
                extUsageAccum = {
                  prompt_tokens: (extUsageAccum?.prompt_tokens ?? 0) + review.usage.prompt_tokens,
                  completion_tokens: (extUsageAccum?.completion_tokens ?? 0) + review.usage.completion_tokens,
                  total_tokens: (extUsageAccum?.total_tokens ?? 0) + review.usage.total_tokens,
                  cost: (extUsageAccum?.cost ?? 0) + (review.usage.cost ?? 0),
                };
              }
              pushTrace({
                step: 'EXTRACTION',
                stepLabel: 'Extraction (review pass)',
                model: modelForExtraction,
                promptPreview: reviewPrompt.slice(0, 1500),
                responsePreview: reviewContent.slice(0, 800),
                tokenUsage: review.usage,
              });
            }
          } catch {
            // keep extractionContent unchanged
          }
        }
      }
